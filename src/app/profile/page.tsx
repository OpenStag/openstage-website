		...
		// Find design and development completion badges by name (adjust names as needed)
			...
			return (
				<div className="max-w-2xl mx-auto p-8">
					...
					{/* Show as many design badges as designs created */}
					<div className="flex gap-4 mb-6">
						{Array.from({ length: designCount }).map((_, idx) => (
							<div key={idx} className="flex flex-col items-center">
								<img src={designBadge?.icon_url || '/design-badge.png'} alt="Design Badge" className="w-16 h-16 mb-1" />
							</div>
						))}
						{/* Show as many development badges as developments completed */}
						{Array.from({ length: developmentCount }).map((_, idx) => (
							<div key={"dev-"+idx} className="flex flex-col items-center">
								<img src={developmentBadge?.icon_url || '/development-badge.webp'} alt="Development Badge" className="w-16 h-16 mb-1" />
							</div>
						))}
					</div>
